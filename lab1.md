...
8. **Завершение и обновление**:
   ...
   - Войдите как admin.
   - Проверьте выдачу ip-адреса выполнив команду `ip address`, у интенфейса eth0 должен появиться адрес.
   - Если адрес отсутствует, измените файл конфигурации сети выполнив команду `sudo nano /etc/network/interfaces`, далее в конец файла допишите строки `auto eth0` и `iface eth0 inet dhcp`, после чего перезагрузите систему выполнив `sudo reboot`
